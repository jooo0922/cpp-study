...
/*
	exception specifier (에러 명시자)


	이 문법은 약간 무의미한 문법일 수도 있는데,
	아래 last() 함수와 같이

	함수 이름 옆에 throw(error type) 을 명시하면,
	'이 함수는 해당 타입의 에러를 던질 수도 있는 가능성이 있는 함수에요'
	라고 선언하는 것과 같다고 함.

	그러나, 이 타입에 뭘 넣건 간에, 
	내부적으로는 결국 throw(...) 로 컴파일되기 때문에
	'어떤 타입의 에러든 던져질 수도 있고 아닐 수도 있고' 정도의 의미라고 함.

	즉, 이걸 함수 이름 옆에 붙인다고 해서
	반드시 해당 타입의 에러를 던져야 하는 것도 아니고,
	심지어는 에러 자체를 반드시 던져야 하는 것도 아님.

	또한, throw() 와 같이 에러 타입을 명시하지 않으면,
	오히려 '에러를 던지지 않는 함수에요' 라는 의미로 선언하는 것이기 때문에,
	빌드 시 warning 이 출력됨.

	여러모로 별 쓸모가 없는 문법이라고 볼 수 있지만 그냥 알아둘 것.
*/
void last() throw(int)
{
	cout << "Last" << endl;
