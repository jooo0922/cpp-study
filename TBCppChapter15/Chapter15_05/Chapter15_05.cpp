...
int main()
{
	...
	{
		/*
			절대 아래와 같이 unique_ptr 을 사용하면 안되는 케이스


			동적으로 메모리가 할당된 Resource 의 주소값을 아래 코드와 같이 
			두 개의 서로 다른 unique_ptr 에 전달하는 건 당연히 안되겠지!

			unique_ptr 자체가
			주소값을 고유한 하나의 스마트 포인터 변수만으로
			관리할 수 있도록 만든 개념인데

			동일한 주소값을
			서로 다른 두 개 이상의 unique_ptr 이
			관리하도록 내버려 둔다는 게 말이 안되겠지!
		*/
		...
		/*
			unique_ptr 을 사용했으면,
			메모리 반납 등의 관리는 모두 스마트 포인터인 unique_ptr 이
			알아서 처리하도록 맡겨야 하는데,

			예전 c++ 코딩하던 버릇이 남아서
			block 이 끝나기 전에 delete 키워드를 명시해버리면,

			이미 unique_ptr 내에서 자동으로 반납해 준 메모리를
			불필요하게 중복 반납해버리려고 하는 불상사가 발생함.
