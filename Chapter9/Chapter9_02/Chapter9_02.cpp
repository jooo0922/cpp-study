...
public:
	...
	*/
	...
	// 입력 스트림도 출력 스트림과 동일한 방식으로 연산자 오버로딩이 가능함!
	// 단, 입력 스트림 오버로딩 시 주의할 점이 있음! 말 그대로 '값을 입력'받아서 Point 클래스 멤버에 할당해줘야 하므로,
	// const Point& 와 같이 입력 매개변수 형태로 두 번째 매개변수를 선언하면 안되고,
	// 출력 매개변수 형태인 일반 참조변수로 선언해줘야 함!
...
int main()
{
	...
	// 이번에는 인스턴스를 생성만 해놓고, 멤버에 초기화할 값들은
	// 오버로딩한 입력 스트림을 통해서 받아보자!
	Point p1, p2;
	cin >> p1 >> p2;
