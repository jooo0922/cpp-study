...
{
    // 구조체 매개변수를 포인터 변수로 받더라도,
    // 그 안의 배열 멤버변수의 메모리 사이즈는 제대로 출력해주는군.
    // 배열이 5개니까, 5개 요소의 주소값 사이즈인 4 bytes * 5 = 20 bytes 가 제대로 출력되어야겠지?
    // 즉, 배열이 클래스나 구조체의 멤버변수로 존재한다면, 해당 클래스나 구조체 타입의 포인터 변수로
    // 매개변수를 선언하더라도, 배열이 포인터 변수로 강제로 변환되지 않음.
    // 위에 printArray() 처럼 배열 자체를 직접 매개변수로 선언했을 때에만 포인터 변수로 강제 변환됨.
    cout << sizeof((*ms).array) << endl; // 20 bytes 
}
...
int main()
{
    ...
    */
    ...
    // 구조체 내에 선언 및 초기화된 배열 멤버변수와 그것의 메모리 사이즈를 확인해보자
    MyStruct ms;
    cout << ms.array[0] << endl; // 9
    cout << sizeof(ms.array) << endl; // 20 bytes

    doSomething(&ms); // 20 bytes > 구조체 매개변수를 포인터 변수로 받을 경우에도 그 안의 배열 멤버변수의 메모리 사이즈를 제대로 출력해주는군.
