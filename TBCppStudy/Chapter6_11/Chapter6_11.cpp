...
int main()
{
    ...
    */
    ...
    // 무시무시한 memory leak 에 대해 배워보자
    // 어딘가 알 수 없는 곳에서, 메모리를 OS 로부터 계속 할당만 받고
    // 받은 메모리를 반납하지 않을 때, 메모리 누수가 발생함!
    while (true)
    {
        int* ptr = new int; // int 타입의 4 bytes 메모리를 반복적으로 할당받음.
        cout << ptr << endl; // 할당받은 메모리의 주소값 출력

        // 메모리 주소값이 담긴 지역변수 ptr(포인터 변수) 은
        // 이 반복 순회 블록이 종료되는 순간 자동으로 메모리를 OS 에 반납함 > ptr 변수 자체는 자동 메모리 이니까!
        // 그렇다면, new int 로 할당받은 메모리 공간의 주소값을 붙잡고 있는 변수도 사라져버리므로,
        // 할당받은 메모리 공간에 대한 레퍼런스 자체가 사라진 상황에서
        // 할당받은 메모리는 계속 남아있는 상태가 되어버림.
        // 이로 인해, 반복문 외부에서라도 delete 연산자로 할당받은 메모리를 OS 에 반납할수도 없게 됨!
        // 이처럼, 해당 메모리에 대한 레퍼런스도 끊긴 상태에서, 메모리를 반납하지 않았다면
        // 지속적으로 메모리 할당이 쌓여서 메모리 누수가 발생하게 되는 것!

        // 그래서 아래와 같이 다음 반복 순회로 넘어가기 전,
        // 할당받은 메모리를 반납해주면 메모리 누수가 발생하지 않음!
        delete ptr;

        /*
            참고로, 이 new 와 delete 로
            메모리를 동적 할당하는 과정은 비교적 느림.

            왜냐하면, 동적 할당이기 때문에
            OS 를 거쳐서 메모리 달라고 요청하는 중간 과정을 거치기 때문.

            그래서, 프로그래밍을 잘 하려면
            가급적 이 new 와 delete 를
            적게 사용하는 방향으로 프로그래밍하는 것이 좋음!
        */
    }
