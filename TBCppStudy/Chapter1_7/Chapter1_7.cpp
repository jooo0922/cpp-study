...
{
	// doSomething 함수 내부의 x 는 123으로 찍히는데,
	// main() 함수 내에서 doSomething 함수 호출부 다음 줄에서 x 는 왜 초기값인 0으로 남아있을까?
	// 그 이유는, main 함수에서 선언된 변수 x 와, doSomething 함수 내의 변수 x 는 서로 다른 메모리 공간을 갖고 있기 때문!
	// 특정 함수의 매개변수로 int x 선언 시, 스택 메모리 공간에 새로운 메모리 공간이 할당된 것과 같기 때문에, 함수 블록 내에서 선언된 매개변수와,
	// 호출부에서 인자로 넘겨주는 변수는 서로 다른 메모리 공간을 가리키는 변수임을 알 수 있다!
	x = 123;
	cout << x << " " << &x << endl; // 123
}
