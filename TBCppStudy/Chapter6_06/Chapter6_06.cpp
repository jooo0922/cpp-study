...
int main()
{
	...
	*/
	...
	// strcat_s() // 두 문자열을 이어붙여줌
	// strcmp_s() // 두 문자열이 동일한 지 비교

	//strcat_s(dest, source);
	cout << strcmp(source, dest) << endl; // strcmp() 는 두 문자열이 같으면 0, 다르면 -1을 출력함.
	// if (strcmp(source, dest) == 0) // 따라서, 두 문자열이 같은지 조건문을 구현하려면 왼쪽처럼 0과 같은지 비교해야 함.

	// c-style 문자열은 배열을 공부하기 좋기 때문에 알아본 것일 뿐,
	// 어디까지나 실전에서는 std::string 을 사용하는 게 훨씬 편하고 사용하기 좋음.
