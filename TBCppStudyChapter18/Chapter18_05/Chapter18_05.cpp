...
int main()
{
	...
	*/
	...
	/*
		이번에는 조금 복잡한 정규 표현식을 만들어 봄.

		총 3개의 정규표현식이 그룹 '()' 으로 묶여있음.

		각 그룹별로 정규표현식을 설명해보자면,

		([0-9]{1}) 는
		0~9 까지의 한 자릿수 숫자가 입력되었는지 검증하는 것이고,

		([-]?) 는
		- 부호가 있어도 되고 없어도 되고 라는 의미임. 
		? 는 어떤 입력값이 입력되어도 되고 안되어도 된다는 뜻임.

		([0-9]{1,4}) 는
		0~9 까지의 숫자가 최소 한자릿수, 최대 네자릿수까지 
		입력되었는지 검증하는 것임.
	*/
