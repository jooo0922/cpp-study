...
/*
	참고로, Timer 로 측정한 코드 실행시간은
	프로그램을 Debug 모드로 빌드하느냐 
	Release 모드로 빌드하느냐에 따라 속도 차이가 매우 큼.

	왜냐하면, Release 모드는 그만큼 불필요한 코드를 압축하여 줄이고
	실행시간을 줄이기 위해 최적화하기 때문!

	그만큼 실제 프로그램을 배포할 때에는 Release 모드로 빌드하므로,
	실제 배포될 프로그램의 정확한 실행시간을 측정하고 싶다면,
	Release 모드로 빌드한 후 측정하는 게 더 정확함!
*/