...
public:
    ...
    * 이때, 주의할 점이 몇 가지 있음.
    * 
    * 부모클래스에서 virtual 키워드로 선언된 멤버함수들은
    * 함수를 실행할 때, stack 메모리에 차곡차곡 쌓아둔 뒤 실행하는 게 아니라,
    * 
    * virtual table 에 저장된 정보들을 바탕으로
    * 이 함수가 어느 부모클래스에서 선언된 함수를 상속받은 것인지 
    * 찾아가는 과정을 거치게 됨.
    * -> 이런 과정을 거치기 때문에 virtual 함수들을 실행할 때마다 느리겠지!
    * 
    * 그래서 호출이 빈번한 함수에서는
    * 가급적이면 virtual 함수를 사용하지 않는 것이 좋음!
    * 
    */
